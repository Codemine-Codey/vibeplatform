// AUTO-GENERATED from taste-design.md — edit the .md file, then re-run scripts/gen-md-strings.mjs
const content = `---
name: taste-design
...
* **IMAGE UNIQUENESS (mandatory):** Every image slot on the page must be a DISTINCT image. Request one unique image per section (hero, each feature, each gallery item, each card) in the image batch — never generate 3 images and repeat them across 10 slots. A single image may appear at MOST twice on the whole page (e.g. a logo mark); everything else is unique. Reusing the same photo for the hero, the about section, and three cards reads as broken/placeholder. If you have 8 image slots, request 8 prompts.
* **HERO TEXT OVER MEDIA (mandatory, the recurring "text blends in" bug):** When headline/body text sits over a photo or gradient, GUARANTEE contrast — never let text blend into the background. Use one of: a darkening scrim (\`bg-black/45\` to \`/60\` overlay, or a bottom-up \`bg-gradient-to-t from-black/70\`), a solid color panel behind the text, or place the text in a separate non-image column (split hero). Pick the text color from the OPPOSITE end of the background's luminance (light text on dark scrim, dark text on light panel). Mentally check the exact headline pixels against the exact background behind them — if it's low-contrast, add the scrim. This applies to nav links over a hero image too.
* **BACKGROUND THEME IS PER-PROJECT (mandatory):** The section backgrounds, surface tints, and overall color temperature must come from THIS brand's palette — not a default cream/white every time. A bold fitness brand ≠ a calm wellness brand ≠ a fintech dashboard. Vary the background system per project: dark editorial, warm paper, cool clinical, saturated brand-color washes — chosen from the brief. Two different projects should not share the same background palette by default.
