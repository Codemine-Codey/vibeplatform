// AUTO-GENERATED from threejs.md — edit the .md file, then re-run scripts/gen-md-strings.mjs
const content = `---
name: threejs
description: 3D and three.js animation — keyframe, skeletal, morph targets, GLTF, procedural motion, animation mixing. Load when the build needs a 3D hero, a WebGL scene, or three.js animation.
---

> PLATFORM SETUP (do this first when using 3D): this project is React + Vite. Use React Three Fiber.
> Add these to package.json dependencies (the platform installs them automatically): "three", "@react-three/fiber", "@react-three/drei".
> Render the scene inside a <Canvas> from @react-three/fiber; use drei helpers (OrbitControls, Environment, useGLTF, Float, etc.). Keep it performant (dispose, suspense for assets). Provide a graceful fallback + respect prefers-reduced-motion.
