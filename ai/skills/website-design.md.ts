// AUTO-GENERATED from website-design.md — edit the .md file, then re-run scripts/gen-md-strings.mjs
const content = `---
name: website-design
...
- **Type-in / counter** — the scaffolded \`<CountUp to="10K+" />\` from \`@/components/blocks\` for stats (it scroll-triggers on view — reliable, and it parses "10K+", "98%", "$2.5M", "10,000" and animates the number while keeping the suffix). Do NOT use raw \`react-countup\` directly: without \`enableScrollSpy\` it does NOT animate on scroll, so stats below the fold silently never count (the "counter isn't working" bug). A subtle type-on for a tagline (sparingly).
...
- **Stat strip** — a band of 3-4 big animated numbers (the scaffolded \`<CountUp to="98%" />\` from \`@/components/blocks\`, NOT raw react-countup) over a token-tinted surface.
