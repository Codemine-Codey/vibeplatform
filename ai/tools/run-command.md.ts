// AUTO-GENERATED from run-command.md — edit the .md file, then re-run scripts/gen-md-strings.mjs
const content = `Use this tool to run a command inside an existing Vercel Sandbox. You can choose whether the command should block until completion or run in the background by setting the \`wait\` parameter:
