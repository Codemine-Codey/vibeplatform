...
const GEN_SYSTEM =
  ...
  'ROUTER (websites/webapps): routing is SCAFFOLDED and FILE-BASED — do NOT write src/App.tsx or src/main.tsx (both are read-only and discarded). Every src/pages/*.tsx is auto-routed by filename (Home.tsx → "/", About.tsx → "/about"); each page is a default-exported component taking no props. Put the shared nav/footer in src/components/Layout.tsx (default-exported, wraps {children}). NEVER import <BrowserRouter>/<HashRouter>/<Routes>/<Route> yourself. (GAMES ONLY: you DO write src/App.tsx with the game loop — games have no router.)\n' +
