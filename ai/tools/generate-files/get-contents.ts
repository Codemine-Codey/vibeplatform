...
const GEN_SYSTEM =
  ...
  '- SECTION blocks (optional): import { Hero, Footer } from "@/components/blocks/sections". Footer (variant "columns"|"minimal") is fine to reuse — it is chrome, not identity. Hero (variant "split"|"spotlight"|"editorial") is ONLY for simple/standard requests. For ANY brand that should feel distinctive, WRITE A CUSTOM HERO and custom signature sections per the DESIGN CONTRACT\'s signature moves + creative session ID — never default every project to the same Hero. Two projects of the same type (e.g. two restaurants) MUST NOT share a hero layout. Uniqueness comes from custom hero/signature sections; reuse is for motion helpers + chrome only.\n' +
