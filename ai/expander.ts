...
- archetype (CRITICAL for variety — pick the ONE that fits this brand best, NEVER default to the safe/plain option): choose from editorial-magazine, bento-grid, swiss-minimal, brutalist-mono, immersive-parallax, glassmorphic-dark, warm-boutique, kinetic-type, retro-print, luxury-serif, playful-rounded, corporate-clean. Match it to the brand's soul — a fine-dining brand → luxury-serif or editorial-magazine; a dev tool → swiss-minimal, bento-grid, or glassmorphic-dark; a creative agency → kinetic-type, brutalist-mono, or immersive-parallax; a boutique/cafe → warm-boutique or retro-print. Two different briefs should rarely get the same archetype. This is what stops every site looking the same — commit hard to the archetype's structural language.
      ...
      tools: {
        create_brief: tool({
          ...
          inputSchema: z.object({
            ...
            archetype: z.enum(['editorial-magazine', 'bento-grid', 'swiss-minimal', 'brutalist-mono', 'immersive-parallax', 'glassmorphic-dark', 'warm-boutique', 'kinetic-type', 'retro-print', 'luxury-serif', 'playful-rounded', 'corporate-clean']).describe('The committed visual archetype that best fits this brand — pick boldly, never the plain default. Drives the whole structural language.'),
            ...
            backgroundTreatment: z.enum(['flat', 'gradient-mesh', 'noise-grain', 'animated-gradient', 'scroll-parallax', 'aurora-glow', 'particles', '3d-scene']).describe('Background depth/motion treatment — bias away from flat; use 3d-scene for wow when it fits'),
