...
- **CRITICAL — no invented Tailwind color classes**: Never write `bg-cafe-coral`, `text-sakura-red`, `border-ocean-blue` or any made-up color name as a Tailwind class in JSX or CSS. Only use color classes from the pre-configured list: Tailwind's built-in palette (slate, gray, red, blue, etc.) OR the scaffold tokens (bg-primary, bg-secondary, bg-muted, bg-accent, bg-background, bg-foreground, bg-border, bg-card, bg-destructive, bg-input, bg-ring). For custom brand colors, define them as CSS vars in `:root` and apply via `style={{ backgroundColor: 'var(--my-color)' }}` or inline CSS.
...
- **DESIGN-TOKEN DISCIPLINE (non-negotiable, this is what makes the design look cohesive):** The PROJECT BRIEF gives you a LOCKED palette of 6 semantic colours. Convert each hex to an `H S% L%` triplet and set the shadcn variables in `src/index.css` `:root` — `--background`, `--card`, `--foreground`, `--muted-foreground`, `--primary`, `--accent` — plus derived `--border`/`--secondary`/`--ring` for contrast. Then in EVERY component use ONLY token classes (`bg-background`, `bg-card`, `text-foreground`, `text-muted-foreground`, `bg-primary`, `text-accent`, `border-border`). NEVER hardcode a colour in a component: no `bg-[#…]`, no `text-white`/`bg-black`, no `text-slate-900`/`bg-gray-50` for brand surfaces. One source of truth = no drift. Define one styled variant each for buttons, cards, and inputs and reuse them across pages.
- **Implement every SIGNATURE MOVE from the brief.** These are the specific, art-directed details (hero treatment, scroll effect, texture, unusual grid) that separate "looks designed" from "looks AI-generated". They are required, not optional.
- **Commit to the brief's LAYOUT ARCHETYPE.** Do not fall back to the same three-stacked-sections template — give each page a distinct composition true to the named archetype.
