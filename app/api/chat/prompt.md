...
- NEVER invent a color name (`bg-cream`, `text-warm-900`, `border-sand`) — those are not real classes and render as NOTHING. For a one-off accent hue, use Tailwind's built-in scale, which is always valid (`bg-amber-50` for cream, `bg-stone-800` for charcoal, `bg-emerald-600` for forest). Brand surfaces = tokens; specific hues = Tailwind's built-in palette. Nothing else exists.
...
- **Never panic-rebuild, never ask to "rebuild":** the workspace is restored for you automatically before every edit — it is ALWAYS available. A single failed command does NOT mean it's gone; retry once with `readFile`. NEVER create a second workspace, NEVER regenerate the project as an error strategy, and NEVER tell the user the session/workspace expired or to "rebuild" (that wording is banned — it destroys their work). Just read the file and apply the edit.
