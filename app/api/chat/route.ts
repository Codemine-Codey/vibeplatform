...
- src/components/GameCanvas.tsx — owns the <canvas>, the requestAnimationFrame loop, and ONE useRef<GameState>. FIT-TO-VIEWPORT (NON-NEGOTIABLE — use the ready-made hook, do NOT hand-write sizing): render the canvas inside a container that fills the space (a w-full h-full div), then use { w, h } = useCanvasFit(canvasRef) imported from @/components/game/engine. That hook sizes the canvas to its parent x devicePixelRatio, re-fits on resize, and scales the 2D context so you DRAW IN LOGICAL PIXELS. Use the returned w/h for EVERY game coordinate (playfield bounds, spawn positions, cell size) so the game ALWAYS fills the preview — never letterboxed, cut off, or scrolling. The App/page root must be h-screen w-screen overflow-hidden so the canvas has a real full-size box. Never use fixed pixel dimensions or window.innerWidth directly.
- WORLD-SCALING (CRITICAL — the whole game must be visible at 100% zoom, no scrolling, no need to zoom out): DERIVE every size from the live { w, h }, never hard-code the board. For a GRID game (snake, tetris, breakout): pick a fixed grid COUNT (e.g. 24 columns), then cellSize = Math.floor(Math.min(w, h) / gridCount) and center the board — never a fixed cellSize in px. For a FREE-SCROLL game (flappy, runner): position the ground, ceiling, obstacle gaps, and player as FRACTIONS of h (e.g. groundY = h * 0.85, gapHeight = h * 0.28), never fixed px. The playfield's outer bounds MUST equal the canvas w/h so walls/edges are on-screen. Recompute all of this from w/h whenever the canvas re-fits. If any part of the game (a wall, the score, the edges) is off-screen at 100% zoom, the scaling is WRONG.
