      ...
      <div className="relative z-10 px-4 pb-24 pt-32">
        <div className="mx-auto max-w-4xl">
          ...
          <Reveal className="mb-24">
            <h2 className="mb-6 flex items-center gap-3 text-2xl font-bold text-[var(--cm-heading)]">
          ...
          {/* Guiding Principles */}
          <Reveal className="mb-24">
            <h2 className="mb-8 flex items-center gap-3 text-2xl font-bold text-[var(--cm-heading)]">
            ...
            <div className="grid gap-6 md:grid-cols-3">
              {PRINCIPLES.map((p) => (
                <div
                  key={p.tag}
                  className="rounded-2xl border border-[var(--cm-border)] bg-[var(--cm-card)] p-6 backdrop-blur transition-colors hover:bg-[var(--cm-card-hover)]"
                >
                  <div className={`mb-4 text-xs font-bold uppercase tracking-wider ${p.tagColor}`}>{p.tag}</div>
                  <h3 className="mb-3 text-lg font-bold text-[var(--cm-heading)]">{p.title}</h3>
                  <p className="text-sm leading-relaxed text-[var(--cm-muted)]">{p.body}</p>
                </div>
              ))}
              {/* Values card */}
              <div className="rounded-2xl border border-[var(--cm-border)] bg-[var(--cm-card)] p-6 backdrop-blur transition-colors hover:bg-[var(--cm-card-hover)]">
                <div className="mb-4 text-xs font-bold uppercase tracking-wider text-amber-500">Values</div>
                <h3 className="mb-3 text-lg font-bold text-[var(--cm-heading)]">Core Pillars</h3>
                <ul className="space-y-2 text-sm text-[var(--cm-muted)]">
                  <li className="flex items-center gap-2">
                    <Zap size={14} className="text-amber-500" />{' '}
                    <strong className="text-[var(--cm-body)]">Innovation:</strong> Push boundaries.
                  </li>
                  <li className="flex items-center gap-2">
                    <Heart size={14} className="text-blue-500" />{' '}
                    <strong className="text-[var(--cm-body)]">Simplicity:</strong> Make tech accessible.
                  </li>
                  <li className="flex items-center gap-2">
                    <Shield size={14} className="text-emerald-500" />{' '}
                    <strong className="text-[var(--cm-body)]">Trust:</strong> Build reliability.
                  </li>
                </ul>
              </div>
            </div>
          ...
          <Reveal>
            <div className="relative overflow-hidden rounded-3xl border border-[var(--cm-panel-border)] bg-[var(--cm-panel)] p-8 md:p-12">
              ...
              <h2 className="relative z-10 mb-6 flex items-center gap-3 text-2xl font-bold text-[var(--cm-panel-heading)]">
                <span className="flex size-8 items-center justify-center rounded-lg bg-blue-500/10 text-blue-500">
                  <Rocket size={18} />
                </span>
                Our Commitment
              </h2>
              <div className="relative z-10 space-y-6 text-lg leading-loose text-[var(--cm-panel-body)]">
                <p>
              ...
              <div className="relative z-10 mt-8">
                <Link
                  href="/signup"
                  className="inline-flex items-center gap-2 rounded-full bg-blue-600 px-6 py-3 text-sm font-bold text-white shadow-lg shadow-blue-900/30 transition-colors hover:bg-blue-500"
                >
                  Start building
                  <Rocket size={16} />
                </Link>
