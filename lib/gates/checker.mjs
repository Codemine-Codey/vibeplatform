...
// Given an importer path + a LOCAL specifier, return the src-relative base path (no
// extension) the import points at — so a caller can create the missing file. Returns
// null for bare package specifiers. e.g. ('src/pages/Home.tsx', '@/components/Timer')
// → 'src/components/Timer'; ('src/pages/Home.tsx', './sections/Contact') → 'src/sections/Contact'.
export function localImportBasePath(importerPath, spec) {
  if (spec.startsWith('@/')) return normalizeRel('src/x', spec.replace(/^@\//, './'))
  if (spec.startsWith('./') || spec.startsWith('../')) return normalizeRel(importerPath, spec)
  return null
}
...
// END-OF-GEN GATE PLANNER (pure, harness-testable): given all src files {path,content},
// return the list of files to CREATE for every unresolved LOCAL import — {createPath, spec,
// importer}. Extension is JSX-aware (imported binding used as <Name → .tsx else .ts, with a
// components/pages/... path fallback). The caller feeds each to generateMissingFile. This is
// the single source of truth shared by the build pipeline's end-of-gen resolve gate and the
// replay harness, so the create-path decision is proven at $0.
export async function plannedMissingFiles(fileList, scaffoldAliases = SCAFFOLD_RESOLVABLE) {
  const withImports = []
  for (const f of fileList) withImports.push({ path: f.path, content: f.content, imports: await extractImports(f.path, f.content) })
  const fileSet = new Set(fileList.map(f => f.path))
  const unresolved = resolveGate(withImports.map(w => ({ path: w.path, imports: w.imports })), fileSet, scaffoldAliases)
  const byPath = new Map()
  for (const u of unresolved) {
    const base = localImportBasePath(u.file, u.specifier)
    if (!base) continue
    const importer = withImports.find(w => w.path === u.file)
    const name = base.split('/').pop() || ''
    const usedAsJsx = name.length > 0 && new RegExp(`<${name}[\\s/>]`).test(importer?.content || '')
    const ext = (usedAsJsx || /\/(components|pages|sections|screens|views|layouts)\//.test('/' + base)) ? '.tsx' : '.ts'
    const createPath = base + ext
    if (fileSet.has(createPath)) continue
    if (!byPath.has(createPath)) byPath.set(createPath, { createPath, spec: u.specifier, importer: u.file })
  }
